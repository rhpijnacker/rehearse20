...
class RTPTransmitter(object):
    ...
    def build_audio_interface(self):
        print('Building audio input bin')
    ...
    def build_transport(self):
        ...
        print('Set receiver to %s:%i' % (self.link_config.receiver_host, self.link_config.port))

        # if self.link_config.multicast:
            # udpsink.set_property('auto_multicast', True)
            # print('Multicast mode enabled')
        bin.add(udpsink)
                ...
                if struct.get_name() == 'level':
                    if self.started is False:
                        ...
                        if len(struct.get_value('peak')) == 1:
                            print('Started mono audio transmission')
                        else:
                            print('Started stereo audio transmission')
                    else:
                        if len(struct.get_value('peak')) == 1:
...
if __name__ == '__main__':
    ...
    class Config(object):
        def __init__(self):
            ...
            self.encoding = 'opus'
            self.bitrate = 128
            self.opus_framesize = 20 # ms
            self.opus_complexity = 9
            self.opus_fec = True
            self.opus_loss_expectation = 0
            self.opus_dtx = False

    transmitter = RTPTransmitter(Config(), AudioInterface())
    transmitter.run()
    print('Transmitter caps: %s' % transmitter.get_caps())
    transmitter.loop()
